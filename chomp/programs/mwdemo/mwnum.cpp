...
Multi-Work demo: Numbers. Copyright (C) 1997-2013 by Pawel Pilarczyk.\n\
...
-c - run as a coordinator and process data locally if no remote worker,\n\
...
int main (int argc, char *argv [])
{
	...
	for (int i = 1; i < argc; ++ i)
	{
		...
		char *value = argv [i] + 2;
		if ((option != 'w') && (option != 'c') && (option != 'C') &&
			(option != 'k') && (option != 'q') && !*value &&
			(i + 1 < argc) && (argv [i + 1] [0] != '-'))
		{
			value = argv [++ i];
		}
	...
	// show help info if neither coordinating nor working
	else
	{
		std::cout << helpinfo << std::endl;
	}
