...
const char *title = "\
FILEDEPS, ver. 0.08. Copyright (C) 1997-2013 by Pawel Pilarczyk.\n\
...
// Skip files already known. Use recursion.
{
	...
	for (int_t i = 0; i < included. size (); ++ i)
	{
...
// Determine on which objects the given one is dependent. Recursive.
{
	...
	for (int_t i = 0; (i < depnames. size ()) && flatobjects; ++ i)
	{
	...
	for (int_t i = 0; i < dependent. size (); i ++)
	{
...
// Write the link dependencies for the given executable.
{
	...
	fstr << "${BIN}" << no_path (name) << "${EXE}: " << "${OBJ}" <<
		no_path (name) << objext;
	for (int_t i = 0; i < objects. size (); i ++)
	...
	for (int_t i = 0; i < objects. size (); i ++)
	{
		fstr << " ${OBJ}" << (keepdirs ? objects [i] :
			no_path (objects [i])) << objext;
	}
	fstr << " ${CCLIB}";
	if (addcapdlibs)
		fstr << " ${CAPDLIB}";
...
// Create the "filedeps" file. Return 0 on success or -1 on error.
{
	...
	for (int_t i = 0; i < allnames. size (); i ++)
	{
	...
	if (flatobjects)
	{
		int_t n = headers. getdomain (). size ();
		for (int_t i = 0; i < n; ++ i)
		{
...
//	}
	...
	for (int_t i = 0; i < prognames. size (); i ++)
	{
	...
	if (f_cur)
	{
		...
		else for (int_t i = 0; i < depslen; ++ i)
		{
			if (buffer [i] == deps [i])
				continue;
