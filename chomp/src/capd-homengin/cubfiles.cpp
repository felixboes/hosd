...
// This file copyright (C) 1997-2013 by Pawel Pilarczyk.
//
...
int cublistfile::readcubes (chomp::homology::CubicalComplex &s) const
{
...
int cublistfile::readcubes (chomp::homology::SetOfCubes &s) const
{
	using namespace chomp::homology;
	...
	bool power2)
{
	...
	int size0 = nbytes << 3;
	chomp::homology::sbug << "size [0] = " << size0 << "\n";
	...
	for (int d = 1; d < _dim; ++ d)
	{
		...
		chomp::homology::sbug << "size [" << d << "] = " << sizes [d] << "\n";
		nbytes *= sizes [d];
		check_bytes *= sizes [d];
		offsetmult. push_back (offsetmult. back () * sizes [d]);
	}
	chomp::homology::sbug << "Allocating " << nbytes << " bytes.\n";
...
The maximal supported dimension is " << chomp::homology::Cube::MaxDim << ".\n";
	return out;
...
int cellistfile::readcubes (chomp::homology::CubicalComplex &s) const
{
	using namespace chomp::homology;
...
chomp::homology::CubicalCell::MaxDim << ".\n";
	return out;
...
int bitcodefile::readcubes (chomp::homology::CubicalComplex &s) const
{
	using namespace chomp::homology;
...
int bitcodefile::readcubes (chomp::homology::SetOfCubes &s) const
{
	using namespace chomp::homology;
...
int winbmpfile::readcubes (chomp::homology::SetOfCubes &s) const
{
	using namespace chomp::homology;
	...
	int c = 0;
	f. invertedpicture ();
	chomp::homology::coordinate coord [2];
	int height = f. picture_height ();
	int width = f. picture_width ();
	for (int y = 0; y < height; ++ y)
	{
...
int bmdfile::count () const
{
	...
	int ch;
	while ((ch = f. get ()) != EOF)
		_count += chomp::homology::bitcountbyte (ch);
	return _count;
} /* bmdfile::count */
...
int bmdfile::readcubes (chomp::homology::SetOfCubes &s) const
{
	using namespace chomp::homology;
...
int cubitmap::count () const
{
	...
	for (const char *ptr = buf; ptr != bufend; ++ ptr)
		_count += chomp::homology::bitcountbyte (*ptr);
	return _count;
} /* cubitmap::count */
...
int cubitmap::readcubes (chomp::homology::SetOfCubes &s) const
{
	using namespace chomp::homology;
