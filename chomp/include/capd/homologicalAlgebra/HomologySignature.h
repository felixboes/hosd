...
class HomologySignature : public std::vector<FGAGrpSignature<ScalarType> >{
  ...
  HomologySignature& operator+=(const HomologySignature& hs2){
    ...
    for(;i<hs2.size();++i){
      this->push_back(hs2[i]); // "this->" added by PwP as suggested by GCC 4.7.0
    }
    return *this;
  }
  ...
  void shrink(){
    ...
    while((--it)>=this->begin() and it->isTrivial()) this->erase(it); // "this->" added by PwP as suggested by GCC 4.7.0
  }
