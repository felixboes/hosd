...
void CubSetT<P_EuclSet>::getNeighborhood3(const Pixel& p, CubSetT& neighborhood) const{
  ...
  do{
    ...
    if(this->containsPixel(q)){// "this->" added by PwP as suggested by GCC 4.7.0
  ...
  while(!pixelsToProcess.empty()){
    ...
    for(int k=0;k<(int)neighbors.size();++k){
      ...
      if(
        ...
        this->containsPixel(neighbors[k]) == 0 // "this->" added by PwP as suggested by GCC 4.7.0
      ){
...
int CubSetT<P_EuclSet>::simpleIntersection(vector<Pixel> pixels) const{
  ...
  // so return false
  for(k=0;k<(int)pixels.size();++k) if(this->containsPixel(pixels[k]))goto found; // "this->" added by PwP as suggested by GCC 4.7.0
...
//  typename P_EuclSet::interval iv[this->embDim()];
  ...
  for(int l=k+1;l<(int)pixels.size();++l){
    if(!this->containsPixel(pixels[l])) continue; // "this->" added by PwP as suggested by GCC 4.7.0
    for(int i=0;i<this->embDim();++i){
...
int CubSetT<P_EuclSet>::acyclicSubspaceOrg(CubSetT& acSubsp) const{
  ...
  for(int k=0;k<(int)neighbors.size();++k){
    if(this->containsPixel(neighbors[k])){ // "this->" added by PwP as suggested by GCC 4.7.0
...
//      nbhd.acyclicSubspaceBCI(acSubsNbhd);  // works worse (smaller acyclic subset is produced)
    ...
    if(acyclicity){
      ...
      for(int k=0;k<(int)neighbors.size();++k){
        if(
          this->containsPixel(neighbors[k]) && // "this->" added by PwP as suggested by GCC 4.7.0
