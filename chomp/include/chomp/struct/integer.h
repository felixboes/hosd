...
class integer
{
        public:
                integer();
                integer( const numbertype& n );
    
		// assignment operator(s)
		integer &operator = (const int n);
                integer &operator = (const numbertype& n);

		// the function "delta": equal to 0 on 0,
		// equal to 1 on invertibles, otherwise > 1
		integer delta (void) const;
...
extern const integer integer_minus_one;
...
inline integer::integer( const numbertype& n )
{
    num = n;
}
...
inline integer &integer::operator = (const numbertype& n)
{
    num = n;
	return *this;
} /* integer::operator = */
...
inline integer integer::delta (void) const
{
        return integer( (mpz_class)abs(num) );
} /* integer::delta */
