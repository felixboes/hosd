...
// Started on August 14, 2003. Last revision: July 27, 2012.
	...
	**gycompl = 0)
{
	...
	if (checkacyclic)
	{
		...
		// reduce the pair (X,A) with acyclicity check
		// NOTE: This doesn't seem to work well, so I have turned
		// this off if the additional bit of 'careful' is set,
		// until I find out what is going on; sorry! [Dec 18, 2010]
		if (!keepacyclic)
		{
			reducepair (Xcubes, Acubes, Fcubmap,
				Xkeepcubes, "X", "A");
		}
...
//	chaincomplex<euclidom> cx (Xcompl. dim (), false /*generators*/);
	...
	int homdimgraph = maxlevel_cx; // cgraph. dim ();
//	while ((homdimgraph >= 0) && (!hom_cx [homdimgraph]. size ()))
//		-- homdimgraph;
	int homdimy = maxlevel_cy; // cy. dim ();
